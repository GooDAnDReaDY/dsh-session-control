...
- **2026-09-18 — Fail-closed модель защиты маршрутов чтения и выгрузки сессий (#33).**
  Серверные маршруты плагина (`/dsh-session-control/transcript`, `titles`,
  `export-batch`, `sizes`, `handoff`, `handoff-summary`) отдают расшифровки и
  служебные данные сессий. Для защиты от утечки через локальную сеть или внешние туннели
  реализована строгая fail-closed проверка источника запроса:
  1. Доступ разрешён только при выполнении одного из условий:
     - запрос поступил с loopback-интерфейса (`127.0.0.1`, `::1`, `localhost`);
     - передан валидный Bearer-токен авторизации в заголовке `Authorization`;
     - передана cookie активной сессии DSH (`token=` или `dsh_token=`);
     - браузер передал метаданные `sec-fetch-site: same-origin` либо заголовок `Origin`
       точно совпадает с заголовком `Host`;
     - при прямом вводе URL в адресную строку браузера (`sec-fetch-site: none`)
       присутствует заголовок `Host`.
  2. Все запросы из внешней сети без явной авторизации или доказательства same-origin
     немедленно отклоняются с кодом `403 Forbidden` (`untrusted request origin`).
  3. Маршруты чтения требуют строго метод `GET`, маршрут суммаризации — строго `POST`.
     Любой другой метод отклоняется с кодом `405 Method Not Allowed` и заголовком `Allow`.
  4. Маршруты не отдают заголовки `Access-Control-Allow-Origin`, исключая кросс-доменное
     чтение из произвольных сайтов.
  5. Согласование с `dsh-lanmode#132`: даже при наличии внешнего шлюза или туннеля,
     плагин обеспечивает защиту на уровне собственных конечных точек (defense-in-depth).
