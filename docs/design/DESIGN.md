...
- **API:** своих HTTP-маршрутов плагин не добавляет. Все изменения идут в
  ядровые `session/rename`, `session/create`, `workspace/insertSessionBefore`,
...
| `RowMenu` | закрепить/открепить, переименовать, переместить ▸, ответвить, скрыть |
| `CollapsedSection` | «Скрытые» и «Архив» |
...
- **Перенос:** подменю со списком папок, текущая отмечена и недоступна.
...
3. **Переложить сессию в правильный проект.** Меню → «Переместить в папку» →
   выбор. Порядок внутри папки сохраняется.
