...
`
    ...
    /** Одна строка сессии. */
    function SessionRow({ node, current, pinned, t, onOpen, onMenu, renaming, onRenameCommit, onRenameCancel }) {
    ...
    function Section({ id, name, count, collapsed, onToggle, children }) {
      return h(
        'div',
    ...
    function SessionListPanel(props) {
      ...
      const [collapsed, toggleСollapsedRaw] = useCollapsed()
      const toggleCollapsed = toggleСollapsedRaw
      ...
      React.useEffect(() => {
        ...
        if (text === '') { setFound(null); return undefined }
        const controller = new AbortController()
        const timer = setTimeout(() => {
          props.searchSessions(text, controller.signal).then(
            (res) => { setFound(res.items.map((x) => x.sessionId)) },
            () => { /* отказ поиска не должен гасить список */ },
          )
        }, 250)
      ...
      const matches = React.useCallback((node) => {
        ...
        return found !== null && found.includes(node.id)
      }, [query, found])

      const pinnedRows = settings.pinned
        .map(nodeOf)
        .filter((n) => n !== undefined && !hiddenSet.has(n.id) && !archivedSet.has(n.id) && matches(n))
      ...
      const groups = (workspaces || []).map((w) => ({
        ...
        rows: (w.sessionIds || [])
          ...
          .map(nodeOf)
          .filter((n) => n !== undefined && matches(n)),
      }))
      ...
      const moveTo = async (workspaceId, sessionId) => {
        setError('')
        try {
          await props.insertSessionBefore(workspaceId, sessionId, undefined)
        } catch (e) {
          setError(t('error.move', { message: (e && e.message) || String(e) }))
        }
      }

      const openMenu = (at, node, intent) => {
        if (intent === 'rename') { setRenamingId(node.id); return }
        const owning = (workspaces || []).find((w) => (w.sessionIds || []).includes(node.id))
        const items = [
          {
            key: 'pin',
        ...
        for (const w of workspaces || []) {
          if (owning !== undefined && w.workspaceId === owning.workspaceId) continue
          items.push({
            key: 'move-' + w.workspaceId,
            label: t('menu.moveTo', { name: w.title }),
            run: () => moveTo(w.workspaceId, node.id),
          })
        }
        ...
        h(
          ...
          !loading
            ? groups.map((g) => h(Section, {
                ...
                name: g.title,
                count: g.rows.length,
                collapsed: collapsed['ws:' + g.id] === true,
                onToggle: toggleCollapsed,
              }, g.rows.length === 0
          ...
          !loading && hiddenRows.length > 0
            ? h(Section, {
                ...
                collapsed: collapsed.hidden !== false,
                onToggle: toggleCollapsed,
          ...
          !loading && archiveRows.length > 0
            ? h(Section, {
                ...
                collapsed: collapsed.archive !== false,
                onToggle: toggleCollapsed,
              },
                h('div', { className: 'dsc-empty' }, t('section.archiveNote')),
                renderRows(archiveRows))
            : null,
        ),
    ...
    function SettingsCard(props) {
      ...
      const body = settings.status === 'loading'
        ? h('div', { className: 'dsc-empty' }, t('card.loading'))
      ...
      return h(
        ...
        h(
          ...
          h('span', { className: 'dsc-section-name' }, t('card.title')),
          h(Chevron, { className: 'dsc-chev' + (open ? '' : ' dsc-chev-collapsed') }),
        ),
        open ? body : null,
      )
    }
    ...
    const en = {
      ...
      'section.archiveNote': 'Archived by the core. They can be opened, but not restored.',
      ...
      'error.move': 'Could not move: {message}',
    ...
    const ru = {
      'head.title': 'Диалоги',
      'head.search': 'Поиск',
      'head.searchPlaceholder': 'Заголовок или текст сообщений',
      'head.add': 'Добавить папку…',
      'section.pinned': 'Закреплённые',
      'section.hidden': 'Скрытые',
      'section.archive': 'В архиве',
      'section.archiveNote': 'Убраны в архив ядром. Открыть можно, вернуть нельзя.',
      'row.blank': 'Новая сессия',
      'row.untitled': 'Без названия',
      'row.running': 'Идёт работа',
      'row.menu': 'Действия с сессией',
      'menu.pin': 'Закрепить',
      'menu.unpin': 'Открепить',
      'menu.rename': 'Переименовать',
      'menu.fork': 'Ответвить сессию',
      'menu.moveTo': 'Переместить в «{name}»',
      'menu.hide': 'Скрыть',
      'menu.unhide': 'Вернуть',
      'menu.archive': 'В архив (навсегда)',
      'confirm.archive': 'Архив необратим: у ядра нет способа вернуть сессию. Продолжить?',
      'empty.noWorkspaces': 'Папок пока нет. Добавьте первую, чтобы начать разговор.',
      'empty.workspace': 'Здесь пока нет диалогов.',
      'empty.search': 'Ничего не найдено.',
      'error.settings': 'Настройки недоступны, поэтому закрепление и скрытие выключены. Список работает.',
      'error.save': 'Не удалось сохранить: {message}',
      'error.move': 'Не удалось перенести: {message}',
      'error.rename': 'Не удалось переименовать: {message}',
      'error.archive': 'Не удалось убрать в архив: {message}',
      'error.addWorkspace': 'Не удалось добавить папку: {message}',
      'age.now': 'только что',
      'age.min': '{n} мин',
      'age.hour': '{n} ч',
      'age.day': '{n} дн',
      'card.title': 'Управление сессиями',
      'card.loading': 'Загрузка…',
      'card.pinnedCount': 'закреплено: {n}',
      'card.hiddenCount': 'скрыто: {n}',
      'card.unhideAll': 'Вернуть все скрытые',
    }
    ...
    exports.apply = function apply(ctx) {
      ...
      ctx.inject(['locale', 'settingsScope'], (uictx) => {
        try {
          uictx.effect(
            () => uictx.locale.register(NS, { en, ru }),
            'dsh-session-control: словари',
          )
