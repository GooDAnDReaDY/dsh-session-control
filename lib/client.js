...
`
    ...
    const LOCAL_SETTINGS_KEY = 'dsc.settings'
    ...
    function readLocalSettings() {
      try {
        ...
        if (typeof console !== 'undefined' && typeof console.warn === 'function') {
          console.warn('[dsh-session-control] failed to read local settings', readErr)
        }
        return {}
      }
    }
    ...
    function writeLocalSettings(patch) {
      try {
        ...
        window.localStorage.setItem(LOCAL_SETTINGS_KEY, JSON.stringify(updated))
        for (const listener of localListeners) {
          try {
            listener()
          } catch (listenerErr) {
            if (typeof console !== 'undefined' && typeof console.warn === 'function') {
              console.warn('[dsh-session-control] local settings listener error', listenerErr)
            }
          }
        }
        return true
        ...
        if (typeof console !== 'undefined' && typeof console.warn === 'function') {
          console.warn('[dsh-session-control] failed to write local settings', writeErr)
        }
        return false
      }
    }

    /** Subscription to settings namespace; tracks status and values with localStorage fallback. */
    function useSettings(scope) {
      ...
      React.useEffect(() => {
        ...
        const onStorage = (e) => {
          if (e.key === LOCAL_SETTINGS_KEY) cb()
        }
      ...
      const isHostReady = hostSnapshot && hostSnapshot.status === 'ready'
      const isHostLoading = hostSnapshot && hostSnapshot.status === 'loading'
      const isHost = isHostReady && hostSnapshot.writable !== false
      const localVal = readLocalSettings()
      const hostVal = (hostSnapshot && hostSnapshot.value) || {}

      const status = isHostLoading ? 'loading' : 'ready'
      const value = isHost ? Object.assign({}, localVal, hostVal) : Object.assign({}, hostVal, localVal)
    ...
    function useCollapsed() {
      ...
      const toggle = React.useCallback((key, byDefault) => {
        setState((prev) => {
          ...
          try {
            window.localStorage.setItem('dsc.collapsed', JSON.stringify(next))
          } catch (noStorage) {
            if (typeof console !== 'undefined' && typeof console.warn === 'function') {
              console.warn('[dsh-session-control] failed to save collapsed state', noStorage)
            }
          }
          return next
        })
    ...
    function SessionListPanel(props) {
      ...
      const matches = React.useCallback((node) => {
        ...
        const text = query.trim().toLowerCase()
        if (text === '') return true
        const derivedTitle = (derived && derived[node.id]) || ''
        if (node.title.toLowerCase().includes(text) || derivedTitle.toLowerCase().includes(text)) return true
        return found !== null && Object.prototype.hasOwnProperty.call(found, node.id)
      }, [query, found, inLabel, derived])
      ...
      const writeList = async (key, next) => {
        setError('')
        try {
          if (scope && typeof scope.set === 'function' && settings.isHost) {
      ...
      React.useEffect(() => {
        ...
        fetch('/dsh-session-control/titles?sessions=' + want.map(encodeURIComponent).join(','))
          ...
          .then((body) => {
            if (dropped) return
            if (body && body.ok === true && body.titles) {
              setDerived((prev) => Object.assign({}, prev, body.titles))
              for (const id of want) {
                if (body.titles[id] === undefined) askedRef.current.delete(id)
              }
            } else {
              for (const id of want) askedRef.current.delete(id)
            }
          })
          .catch(() => {
            for (const id of want) askedRef.current.delete(id)
    ...
    function SettingsCard(props) {
      ...
      const scope = React.useMemo(() => {
        try {
          ...
          if (typeof console !== 'undefined' && typeof console.warn === 'function') {
            console.warn('[dsh-session-control] unexpected configForms.get error', configErr)
          }
        }
        return undefined
      }, [ctx])
      ...
      const saveSetting = (k, v) => {
        if (scope && typeof scope.set === 'function' && settings.isHost) {
          scope.set(k, v).catch(() => writeLocalSettings({ [k]: v }))
        } else {
          writeLocalSettings({ [k]: v })
        }
      }
          ...
          : h('div', null,
              ...
              h('label', { className: 'dsc-card-toggle' },
                h('input', {
                  ...
                  onChange: (e) => saveSetting('hideBlank', e.currentTarget.checked),
                }),
          ...
          const browserInjected = () => ({
            ...
            forkSession: (sessionId) => {
              ...
              if (uiWs && typeof uiWs.forkSession === 'function') {
                uiWs.forkSession(sessionId).catch((forkErr) => {
                  if (typeof console !== 'undefined' && typeof console.warn === 'function') {
                    console.warn('[dsh-session-control] forkSession failed', forkErr)
                  }
                })
              } else {
                sessions.fork({ sessionId, increaseTitle: true })
                  ...
                  .catch((forkErr) => {
                    if (typeof console !== 'undefined' && typeof console.warn === 'function') {
                      console.warn('[dsh-session-control] fallback sessions.fork failed', forkErr)
                    }
                  })
              }
            },
