...
`
      ...
      React.useEffect(() => {
        ...
        const timer = setTimeout(() => {
          searchRef.current(text, controller.signal).then(
            (res) => { if (!cancelled) { setFound(res.items.map((x) => x.sessionId)); setSearchFailed('') } },
            (e) => {
    ...
    const ru = {
      ...
      'section.archiveNote': 'Убраны в архив ядром: открываются на чтение, вернуть пока нельзя.',
      ...
      'error.search': 'Поиск не удался: {message}',
      ...
      'viewer.you': 'Вы',
      'viewer.agent': 'Агент',
      'viewer.tool': 'инструмент: {name}',
      'viewer.loading': 'Загружаю расшифровку…',
      'viewer.empty': 'В этой сессии нет сообщений.',
      'viewer.unreadable': 'Старый формат лога — ядро отказывается его читать. Разговор цел на диске, но показать его здесь нельзя.',
      'viewer.truncated': 'Показаны последние {shown} из {total} сообщений.',
      'viewer.failed': 'Не удалось прочитать сессию: {message}',
