...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    class UiWorkspaceService extends cordis.Service {
      ...
      /**
       * Initiate session: target directory, current directory, or last active.
       *
       * If no folders exist, clears active selection so the conversation view
       * shows the initial empty prompt for creating a folder.
       */
      ...
      /**
       * Once per boot, open the last active folder and deselect archived sessions.
       *
       * Initial selection runs exactly once: `initial` avoids re-triggering
       * after user navigates away. On failure reverts to `waiting` to retry.
       */
      watchNavigation() {
        let initial = 'waiting'
        let disposed = false
      ...
      /**
       * @returns true if selection was cleared from an archived session.
       */
