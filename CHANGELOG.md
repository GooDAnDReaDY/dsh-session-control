...
### Security
- **Строгая аутентификация маршрутов (fail-closed)**: внешние (non-loopback) запросы больше не принимают подделываемые браузерные признаки (`Sec-Fetch-Site: same-origin`, совпадение `Origin`/`Host` или неподтверждённые regex cookie `dsh-auth-*`). Доступ разрешается только при валидации через ядровый сервис `connection.requestRejection(req)` DSH либо по валидному Bearer-токену; в противном случае запросы отклоняются с 403 Forbidden (#57).

### Fixed
- **Устранение пустых catch и диагностическое логирование**: все пустые блоки `catch` в `lib/client.js` снабжены явной диагностической обработкой и выводом в `console.warn`; обязательный `dsh-plugin-preflight` завершается с результатом `FAIL=0` (#61).
- **Обновление версии апдейтера**: версия в карточке настроек обновлена до `0.2.6`.

## [0.2.5] — 2026-09-23

### Fixed
- **Бесшовный fallback настроек в localStorage**: при доступе по сети или через прокси плагин прозрачно сохраняет и читает настройки (закрепление, скрытие, метки, пороги) из localStorage, устраняя аварийный красный баннер и сохраняя полную работоспособность на удалённых клиентах.
- **Безопасность маршрутов (isTrustedOrigin)**: безусловная блокировка запросов Sec-Fetch-Site: cross-site даже с loopback IP; запрет слепого доверия к неподписанным/произвольным токенам (#57).
- **Батчинг и кэширование названий сессий**: согласован лимит батча (30), предотвращено голодание очереди askedRef, устранено кэширование пустых названий (#53).
- **Разделение пустых сессий и повреждённых логов**: пустые сессии отдают empty: true вместо ложного unreadable: true, выводя корректную заглушку (#54).
- **Поиск по вычисленным названиям**: мгновенный фильтр сессий учитывает derivedTitle (#55).
- **Экспорт сессий и документация**: заголовок Content-Disposition: attachment, версия апдейтера 0.2.5, документирование parseTranscript (#56).

## [0.2.4] — 2026-09-22

### Fixed
- **Миграция настроек на configForms API**: клиент переведён со снятого сервиса `settingsScope` на поддерживаемый `configForms` API в DSH 0.1.7+; клиентский файбер активируется без ожидания отсутствующей службы, карточка настроек корректно считывает снимок, обновляет значения и отображает недоступные состояния (#58).
